...
import { 
  ...
  HardHat, Factory, Hospital, Wifi, Briefcase, Car,
  Code, Anchor, GitCommit
} from "lucide-react";

// --- Custom Component: System Architecture Card (for Archetypes) ---
...
const SystemArchitectureCard: React.FC<SystemCardProps> = ({ sysId, title, subtitle, specs, components, icon }) => (
  <div className="border border-[#DC2626]/40 p-6 sm:p-8 bg-gray-900/50 backdrop-blur-sm shadow-2xl transition-all duration-300 hover:border-[#DC2626] hover:shadow-red-900/50 group flex flex-col justify-between">
    <div>
      ...
      {/* Components / ASSET MATRIX */}
      <p className="text-sm font-mono text-gray-300 mb-2">ARCHITECTED ASSET MATRIX // CORE_COMPONENTS:</p>
      <ul className="space-y-1 mb-4">
        {components.map((comp, index) => (
          <li key={index} className="flex justify-between items-center text-sm text-gray-400 font-mono border-b border-gray-800 pb-1">
    ...
    <div className="mt-6 flex justify-between items-center">
      <Link href="#" className="flex items-center text-white bg-[#DC2626] py-2 px-4 text-sm font-semibold hover:bg-red-700 transition duration-300">
...
// --- Custom Component: Process Step (for 6-Phase Protocol) ---
...
export default function Solutions() {
  ...
  const deploymentProtocol = [
    { phase: "PHASE_01 // RECONNAISSANCE", title: "SITE ASSESSMENT & AUDIT", detail: "In-depth load analysis, grid stability auditing, site structural evaluation, and regulatory compliance check.", icon: <Search size={20} className="text-white" /> },
    { phase: "PHASE_02 // TACTICAL PLANNING", title: "SYSTEM ARCHITECTURE DESIGN", detail: "Custom CAD/BIM layout, electrical schematic generation, component selection, and full system redundancy modeling.", icon: <Code size={20} className="text-white" /> },
    { phase: "PHASE_03 // PROCUREMENT & CUSTOMIZATION", title: "IN-HOUSE PANEL FABRICATION", detail: "Manufacturing of custom ATS, PF, and LT panels up to 3000kVA. Asset procurement and quality verification.", icon: <Factory size={20} className="text-white" /> },
    { phase: "PHASE_04 // DEPLOYMENT", title: "INSTALLATION & INFRASTRUCTURE", detail: "Ground floor, rooftop, and multi-floor rigging and installation. Cable routing, power distribution, and auxiliary system integration.", icon: <HardHat size={20} className="text-white" /> },
    { phase: "PHASE_05 // ACTIVATION", title: "COMMISSIONING & LOAD TESTING", detail: "System-wide integration testing, live load bank testing, performance verification, and operator training/handover.", icon: <Anchor size={20} className="text-white" /> },
    { phase: "PHASE_06 // SUSTAINMENT", title: "MAINTENANCE & LIFECYCLE SUPPORT", detail: "24/7 emergency response, scheduled preventive maintenance, remote monitoring, and genuine parts supply contracts.", icon: <GitCommit size={20} className="text-white" /> },
  ];

  const caseStudies = [
    { 
      title: "Metro Healthcare Facility - 850kVA Redundant System", 
      detail: "Deployed dual-redundant 850kVA system with critical ATS controllers for life-safety compliance. Integrated with hospital BMS for seamless failover and silent operation enclosures.",
      sector: "Healthcare", capacity: "850kVA (N+1)",
      link: "#", // Placeholder
    },
    { 
      title: "National Bank ATM Network - Distributed Power Grid", 
      detail: "Engineered and deployed compact, remote-monitored power cores across 60+ branch locations using hybrid gas/diesel units for maximum efficiency and uptime for mission-critical banking operations.",
      sector: "Banking & Finance", capacity: "100kVA - 300kVA",
      link: "#", // Placeholder
    },
    { 
      title: "Textile Manufacturing Plant - 2.2MW Synchronized Array", 
      detail: "Installed and synchronized a 3-unit 2.2MW array to support continuous, heavy-load production lines. Included custom PF and LT distribution panels manufactured in-house.",
      sector: "Manufacturing", capacity: "2.2MW (Sync)",
      link: "#", // Placeholder
    },
  ];

  const engineeringCapabilities = [
    { title: "Custom Panel Manufacturing", desc: "ATS, PF, LT Panels up to 3000kVA. In-house fabrication ensures seamless integration and quality control.", icon: <Settings size={28} /> },
    { title: "Complex Installations", desc: "Rooftop rigging, multi-floor cable routing, and integration into existing industrial infrastructure. Certified for challenging environments.", icon: <HardHat size={28} /> },
    { title: "System Integration", desc: "Synchronization of multiple units, SCADA/BMS linking, and load shedding automation for intelligent power management.", icon: <Cpu size={28} /> },
  ];
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="relative h-screen flex items-center justify-center overflow-hidden">
        <video
          ...
          src="/videos/industrial-activity-loop.mp4" // Use a relevant video
        >
        ...
        <div className="relative z-10 text-center p-4">
          ...
          <div className="mt-10 flex justify-center space-x-4">
            ...
            <Link href="/store" className="flex items-center border border-white text-white py-3 px-8 text-lg font-semibold hover:bg-white/10 transition duration-300">
      ...
      <section className="py-20 bg-gray-950/90 border-t border-b border-gray-800">
        <div className="container mx-auto px-4">
          ...
          <p className="text-lg text-[#DC2626] font-mono mb-12">PRE-ENGINEERED ARCHETYPE DEPLOYMENT / SELECT YOUR MISSION</p>
      ...
      {/* --- 3. TURNKEY DELIVERY PROTOCOL --- */}
      <section className="py-20 bg-gray-900">
        <div className="container mx-auto px-4">
          <h3 className="text-4xl font-bold italic text-white mb-3">DEPLOYMENT PROTOCOL</h3>
          <p className="text-lg text-[#DC2626] font-mono mb-12">THE SIX-PHASE MISSION SEQUENCE / END-TO-END TURNKEY DELIVERY</p>
      ...
      <section className="py-20 bg-gray-950/90">
        <div className="container mx-auto px-4">
          ...
          <p className="text-lg text-[#DC2626] font-mono mb-12">THE APS ADVANTAGE / IN-HOUSE PRECISION ENGINEERING</p>
      ...
      <section className="py-20 bg-gray-900 border-t border-b border-gray-800">
        <div className="container mx-auto px-4">
          ...
          <p className="text-lg text-[#DC2626] font-mono mb-12">VERIFIED DEPLOYMENTS / REAL-WORLD PROBLEM SOLVING</p>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {caseStudies.map((study, index) => (
              <div key={index} className="p-6 bg-gray-950/70 border border-[#DC2626]/40 shadow-lg">
                <p className="text-xs font-mono text-[#DC2626] mb-1 uppercase">{study.sector} // {study.capacity}</p>
                <h4 className="text-xl font-bold text-white mb-3">{study.title}</h4>
                <p className="text-gray-300 mb-4 text-sm">{study.detail}</p>
                <Link href={study.link} className="flex items-center text-white text-sm hover:text-[#DC2626] transition duration-300">
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div className="p-8 border border-white/20 hover:border-[#DC2626] transition duration-300">
              ...
              <p className="text-gray-400 mb-4">Full turnkey design, custom panel fabrication, installation, and long-term sustainment contracts. Optimized for total system lifecycle and mission-critical uptime.</p>
              <ul className="text-sm text-gray-300 space-y-1 text-left inline-block">
                <li><CheckCircle2 size={16} className="inline mr-2 text-green-500" /> Custom Capacity (Up to 3000kVA+)</li>
                <li><CheckCircle2 size={16} className="inline mr-2 text-green-500" /> Full System Ownership & Warranty</li>
                <li><CheckCircle2 size={16} className="inline mr-2 text-green-500" /> Custom Panel Manufacturing</li>
              </ul>
            </div>
            <div className="p-8 border border-white/20 hover:border-[#DC2626] transition duration-300">
              <Car size={48} className="text-[#DC2626] mx-auto mb-4" />
              <h4 className="text-2xl font-bold mb-2">TEMPORARY RENTAL DEPLOYMENT</h4>
              <p className="text-gray-400 mb-4">Rapid deployment of portable units for construction, event coverage, or short-term backup requirements. Flexible contracts and rapid response setup.</p>
              <ul className="text-sm text-gray-300 space-y-1 text-left inline-block">
                <li><CheckCircle2 size={16} className="inline mr-2 text-green-500" /> Pre-Engineered Portable Units</li>
                <li><CheckCircle2 size={16} className="inline mr-2 text-green-500" /> Rapid Mobilization & Setup</li>
                <li><CheckCircle2 size={16} className="inline mr-2 text-green-500" /> Flexible Contract Duration</li>
              </ul>
            </div>
          </div>
        </div>
